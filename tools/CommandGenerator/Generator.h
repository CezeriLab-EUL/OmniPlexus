...
private:
    ...
    static std::string generateDeviceManifestContent(const std::vector<json> &allData) {
        std::ostringstream out;
        ...
        out << "// AUTO-GENERATED BY SmartDrive CommandGenerator - DO NOT EDIT\n";
        out << "//\n";
        out << "// Maps each device's typeShift to its display name and the list of\n";
        out << "// command IDs it supports. Used by the PC console to build the\n";
        out << "// device/command menus at runtime without needing compile-time type knowledge.\n";
        out << "//\n\n";
        ...
        out << "};\n\n";
        out << "// Returns the full device manifest. Generated once as a static local.\n";
        ...
        for (size_t d = 0; d < allData.size(); ++d) {
            ...
            out << "        // \u2500\u2500 " << deviceName << " (typeShift=" << typeShift << ")\n";
            out << "        {\n";
            out << "            " << typeShift << ",\n";
            out << "            \"" << deviceName << "\",\n";
            out << "            {\n";
            ...
            if (data.contains("telemetry") && data["telemetry"].is_array() && !data["telemetry"].empty()) {
                out << "                // auto-generated telemetry request commands\n";
                for (const auto &src: data["telemetry"]) {
            ...
            if (data.contains("settings") && data["settings"].is_array() && !data["settings"].empty()) {
                out << "                // auto-generated setting GET/SET commands\n";
                for (const auto &setting: data["settings"]) {
                    if (!setting.contains("name")) continue;
                    const std::string name = setting["name"].get<std::string>();
                    out << "                " << deviceName << "CommandType::GET_SETTING_" << name << ",\n";
                    out << "                " << deviceName << "CommandType::SET_SETTING_" << name << ",\n";
                }
...
public:
                         ...
                         const std::string &sourceOutputDir)
    {
        ...
        // Generate DeviceManifest.h
        const std::string deviceManifestPath = headerDir + "DeviceManifest.h";
        writeFile(deviceManifestPath, generateDeviceManifestContent(allData));
        std::cout << "  ✓ Generated: " << deviceManifestPath << "\n";
        ...
        for (const auto &data : allData)
        {
            ...
            std::cout << "  ✓ Generated: " << registerPath << "\n";
        }
    }
};
