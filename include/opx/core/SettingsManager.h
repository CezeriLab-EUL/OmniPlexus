...
public:
    using SettingChangedCallback = void (*)(uint16_t settingID, const ValueSource &newValue, void *context);
...
private:
    ...
    void fireCallbacks(uint16_t settingID, const ValueSource &newValue, int16_t idx) {
#ifndef OPX_PLATFORM_AVR
...
public:
    ...
    bool registerSetting(uint16_t settingID, ValueType type) {
        ...
        for (uint8_t i = 0; i < MAX_SETTINGS; i++) {
            if (!entries[i].active) {
                ...
                entries[i].current.initDefault(type); // sets type and zero-initializes data
                count++;
                return true;
            }
        }
    ...
    bool update(uint16_t settingID, const ValueSource &value, bool broadcast = false) {
        const int16_t idx = findIndex(settingID);
        if (idx < 0) {
    ...
    bool handleCommand(const Command &cmd, uint8_t transportID) {
        ...
        const uint16_t settingID = static_cast<uint16_t>((typeShift << 8) | localID);
...
#ifndef OPX_PLATFORM_AVR
    bool attachCallback(uint16_t settingID, SettingChangedCallback cb, void *context = nullptr) {
        const int16_t idx = findIndex(settingID);
        if (idx < 0) {
