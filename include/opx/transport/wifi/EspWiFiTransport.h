...
public:
    explicit EspWiFiTransport(uint16_t port) : role(WiFiRole::SERVER) {
        ...
        server->begin();
        LOG(LogLevel::OP_INFO, "WiFi transport: server started and listening for connections");
    }

    EspWiFiTransport(const char *host, uint16_t port, uint8_t maxReconnectAttempts = 5,
                     uint32_t reconnectDelayMs = 2000) : role(WiFiRole::CLIENT), targetHost(host), targetPort(port),
        ...
        if (client.connect(targetHost, targetPort)) {
            ...
            LOG(LogLevel::OP_ERROR, "WiFi transport: failed to connect to the server. Will retry");
        }
    }
    ...
    bool send(const SerializedData &data) override {
        if (!clientConnected) {
            LOG(LogLevel::OP_WARNING, "WiFi transport: no client connected, cannot send data");
            return false;
        }
    ...
    void accumulate() override {
        ...
        if (role == WiFiRole::CLIENT && !clientConnected) {
            ...
            if (now - lastReconnectAttemptMs < reconnectDelayMs) return;

            lastReconnectAttemptMs = now;
            reconnectAttempts++;

            LOG(LogLevel::OP_WARNING, "WiFi transport: attempting reconnect to the server...");

            if (reconnectAttempts > maxReconnectAttempts) {
                giveUp = true;
                LOG(LogLevel::OP_ERROR, "WiFi transport: failed to reconnect, giving up");
                return;
            }
