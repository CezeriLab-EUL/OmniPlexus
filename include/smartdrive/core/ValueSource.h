...
public:
    ...
    void pack(T value)
    {
        ...
        if constexpr (std::is_same_v<T, uint8_t>)
            vtype = ValueType::UINT8;
        else if constexpr (std::is_same_v<T, int8_t>)
            vtype = ValueType::INT8;
        else if constexpr (std::is_same_v<T, float>)
            vtype = ValueType::FLOAT;
        else if constexpr (std::is_same_v<T, uint16_t>)
            vtype = ValueType::UINT16;
        else if constexpr (std::is_same_v<T, int16_t>)
            vtype = ValueType::INT16;
        else if constexpr (std::is_same_v<T, uint32_t>)
            vtype = ValueType::UINT32;
    ...
    T unpack() const
    {
        ...
        if constexpr (std::is_same_v<T, uint8_t>)
        {
            if (typeField != static_cast<uint8_t>(ValueType::UINT8))
                return 0;
        }
        else if constexpr (std::is_same_v<T, int8_t>)
        {
            if (typeField != static_cast<uint8_t>(ValueType::INT8))
                return 0;
        }
        else if constexpr (std::is_same_v<T, float>)
        {
            if (typeField != static_cast<uint8_t>(ValueType::FLOAT))
                return 0.0f;
        }
        else if constexpr (std::is_same_v<T, uint16_t>)
        {
            if (typeField != static_cast<uint8_t>(ValueType::UINT16))
                return 0;
        }
        else if constexpr (std::is_same_v<T, int16_t>)
        {
            if (typeField != static_cast<uint8_t>(ValueType::INT16))
                return 0;
        }
        else if constexpr (std::is_same_v<T, uint32_t>)
        {
            if (typeField != static_cast<uint8_t>(ValueType::UINT32))
                return 0;
        }
        else if constexpr (std::is_same_v<T, int32_t>)
        {
            if (typeField != static_cast<uint8_t>(ValueType::INT32))
                return 0;
        }
