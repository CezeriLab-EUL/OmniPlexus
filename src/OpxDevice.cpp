...
bool OpxDevice::beginWiFi(uint16_t port, uint32_t stackSize) {
    ...
    if (!addTransport(transport, OpxDeviceTransportID::OPX_WIFI)) return false;
    if (listenTaskHandle == nullptr) {
        listenTaskShouldStop = false;
        // xTaskCreate(opxListenTask, "OpxListen", stackSize, this, 1, &listenTaskHandle);
...
bool OpxDevice::beginHttpServer(uint16_t port, uint32_t stackSize) {
    ...
    if (!addTransport(transport, OpxDeviceTransportID::OPX_HTTP)) return false;
    if (listenTaskHandle == nullptr) {
        listenTaskShouldStop = false;
        // xTaskCreate(opxListenTask, "OpxListen", stackSize, this, 1, &listenTaskHandle);
        xTaskCreatePinnedToCore(
...
bool OpxDevice::connectWiFi(const char *host, uint16_t port,
    ...
    auto *transport = new EspWiFiTransport(host, port, maxReconnectAttempts, reconnectDelayMs);
    if (!addTransport(transport, OpxDeviceTransportID::OPX_WIFI)) return false;
    if (listenTaskHandle == nullptr) {
        listenTaskShouldStop = false;
        // xTaskCreate(opxListenTask, "OpxListen", 4096, this, 1, &listenTaskHandle);
        xTaskCreatePinnedToCore(
            opxListenTask,
            "OpxListen",
            4096,
...
#ifdef ESP32
#ifdef CDNC_MASTER
if (activeSlotCount== 0 && !cdncActive) stopListenTask();
#else
if (activeSlotCount== 0) stopListenTask();
#endif
#endif
}
...
delete settingsManager; settingsManager=nullptr;
    delete telemetryManager; telemetryManager=nullptr;
...
#ifdef ESP32
if (listenTaskHandle== nullptr) cm->listen();
#else
cm->listen();
#endif
...
cm->processResponses();
    ...
    if (telemetryManager) telemetryManager->send();
    ...
    if (heartbeatReceived &&!connectionLost) {
        ...
        if (elapsed >= heartbeatTimeoutMs) {
            connectionLost = true;
            if (connectionLostCallback) connectionLostCallback();
        }
    }

#ifdef CDNC_SLAVE
if (_cdncSlaveActive &&_cdncSlaveManager) _cdncSlaveManager->tick();
#endif
}
...
uint8_t OpxDevice::extractTypeShift(const RawData &frame) {
    ...
    const ProtocolConstants::FrameType type = ProtocolConstants::decodeType(frame.data[0]);
    switch (type) {
...
void OpxDevice::handleForwarding(const TaggedFrame &frame) {
    ...
    if (!isForMe || isProtocolLevel) {
        ...
        for (uint8_t i = 0; i < MAX_FORWARDING_PAIRS; i++) {
            ...
            if (src == a) tm.send(toForward, b);
            else if (src == b) tm.send(toForward, a);
        }
    }
}
...
bool OpxDevice::isDeviceConnected(uint8_t typeShift) const { return deviceRegistry.isConnected(typeShift); }
uint8_t OpxDevice::transportIDFor(uint8_t typeShift) const { return deviceRegistry.transportIDFor(typeShift); }

// ─────────────────────────────────────────────────────────────────────────────
// Heartbeat
// ─────────────────────────────────────────────────────────────────────────────

void OpxDevice::onConnectionLost(ConnectionLostHandler callback) { connectionLostCallback = callback; }
void OpxDevice::setHeartbeatTimeout(uint32_t timeoutMs) { heartbeatTimeoutMs = timeoutMs; }
...
bool OpxDevice::updateTelemetry(uint16_t sourceID, const ValueSource &value) {
    if (!telemetryManager) {
        LOG(LogLevel::OP_WARNING, "OpxDevice: updateTelemetry() before registerTelemetry()");
        return false;
    }
...
bool OpxDevice::sendTelemetryNow(uint16_t sourceID) {
    if (!telemetryManager) {
        LOG(LogLevel::OP_WARNING, "OpxDevice: sendTelemetryNow() before registerTelemetry()");
        return false;
    }
...
bool OpxDevice::updateSetting(uint16_t settingID, const ValueSource &value, bool broadcast) {
    if (!settingsManager) {
        LOG(LogLevel::OP_WARNING, "OpxDevice: updateSetting() before registerSetting()");
        return false;
    }
    return settingsManager->update(settingID, value, broadcast);
}
bool OpxDevice::attachSettingCallback(uint16_t settingID, SettingsManager::SettingChangedCallback cb, void *context) {
    if (!settingsManager) {
        LOG(LogLevel::OP_WARNING, "OpxDevice: attachSettingCallback() before registerSetting()");
        return false;
    }
...
void OpxDevice::broadcastAllSettings() { if (settingsManager) settingsManager->broadcastAll(); }
void OpxDevice::broadcastOneSetting(uint16_t settingID) {
    if (settingsManager) settingsManager->broadcastOne(settingID);
}
...
void OpxDevice::commandBridge(const Command &cmd,
    ...
    if (cmd.commandType == ProtocolConstants::HEARTBEAT_COMMAND) {
        ...
        if (device->cm) device->cm->dispatch(ack, sourceTransportID);
    ...
    if (cmd.commandType == ProtocolConstants::HEARTBEAT_ACK) {
        ...
        if (device->_heartbeatAckHook)
            device->_heartbeatAckHook(cmd, sourceTransportID, device->_heartbeatAckHookCtx);
        return;
    }
    ...
    if (device->commandHandler) {
        device->commandHandler(cmd, seqNum, sourceTransportID, device->commandHandlerContext);
    }
}
...
void OpxDevice::responseBridge(const CommandResponse &response,
    ...
    if (device->responseHandler)
        device->responseHandler(response, sourceTransportID, device->responseHandlerContext);
}
...
void OpxDevice::telemetryBridge(const Telemetry &telemetry,
    ...
    if (device->telemetryHandler)
        device->telemetryHandler(telemetry, sourceTransportID, device->telemetryHandlerContext);
}
