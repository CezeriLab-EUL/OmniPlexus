...
private:
  ...
  static void validateParam(const json &param, const std::string &cmdName,
                            int paramIndex, ValidationResult &result) {
    if (!param.contains("name") || !param["name"].is_string() ||
        param["name"].get<std::string>().empty()) {
      result.addError("Command '" + cmdName + "' param[" +
    ...
    if (!param.contains("type") || !param["type"].is_string()) {
      result.addError("Command '" + cmdName + "' param '" + paramName +
      ...
      if (!VALID_TYPES.count(type)) {
        result.addError("Command '" + cmdName + "' param '" + paramName +
                        ...
                        "'. Valid types are: UINT8, INT8, FLOAT, UINT16, "
                        "INT16, UINT32, INT32, STRING");
      }
    }
    ...
    if (!param.contains("description") || !param["description"].is_string() ||
      ...
      result.addWarning("Command '" + cmdName + "' param '" + paramName +
                        "' is missing the 'description' field");
    }

    if (param.contains("type") && param["type"].is_string() &&
        param["type"].get<std::string>() == "STRING") {
      if (!param.contains("maxLength") ||
          !param["maxLength"].is_number_unsigned()) {
        result.addError("Command '" + cmdName + "' param '" + paramName +
                        "' is missing a valid 'maxLength' field (must be a "
                        "positive integer)");
      }
      ...
      if (param.contains("required") && !param["required"].get<bool>() &&
        ...
        const size_t maxLen =
            param.contains("maxLength") ? param["maxLength"].get<size_t>() : 0;
        if (defaultVal.length() > maxLen) {
          result.addError("Command '" + cmdName + "' param '" + paramName +
                          "' has default value '" + defaultVal +
                          "' that exceeds maxLength (" +
  ...
  static void validateParamOrdering(const json &params,
    ...
    for (size_t i = 0; i < params.size(); ++i) {
      ...
      if (!isRequired) {
        ...
        if (i != params.size() - 1) {
          result.addError("Command '" + cmdName + "' has optional parameter '" +
                          paramName +
                          "' that is not the last parameter. Optional "
        ...
        if (!param.contains("default") || !param["default"].is_string() ||
          ...
          result.addError("Command '" + cmdName + "' has optional parameter '" +
                          paramName +
                          "' without a 'default' value. All optional "
                          "parameters must have defaults.");
        }
      } else {
        // Required parameter
        if (foundOptional) {
          result.addError("Command '" + cmdName + "' has required parameter '" +
                          paramName +
                          "' after an optional parameter. All required "
                          "parameters must come before optional ones.");
        }
      }
    }

    if (optionalCount > MAX_OPTIONAL_PARAMS) {
      result.addError("Command '" + cmdName + "' has " +
                      std::to_string(optionalCount) +
                      " optional parameters. Maximum is " +
                      std::to_string(MAX_OPTIONAL_PARAMS) +
  ...
  static void validateTelemetrySource(const json &source, int index,
                                      ValidationResult &result) {
    ...
    if (!source.contains("id") || !source["id"].is_number_unsigned()) {
      ...
      if (id < MIN_ID) {
        result.addError(label + " has an invalid 'id' value (" + toHex(id) +
    ...
    if (!source.contains("type") || !source["type"].is_string()) {
      ...
      if (!VALID_TYPES.count(type)) {
        result.addError(label + " has unsupported type '" + type +
                        "'. Valid types are: UINT8, INT8, FLOAT, UINT16, "
                        "INT16, UINT32, INT32, STRING");
      }
    }
    ...
    if (source.contains("trigger")) {
      ...
      if (triggerType != "onChange" && triggerType != "periodic" &&
          triggerType != "onRequest") {
        result.addError(label + " 'trigger.type' has invalid value '" +
                        triggerType +
      ...
      if (triggerType == "periodic") {
        if (!trigger.contains("intervalMs")) {
          result.addError(
              label +
              " 'trigger.type' is 'periodic' but 'intervalMs' is missing");
        } else if (!trigger["intervalMs"].is_number_unsigned()) {
          result.addError(label +
                          " 'trigger.intervalMs' must be a positive integer");
        } else {
          ...
          if (intervalMs > 65535) {
            result.addError(label +
                            " 'trigger.intervalMs' must be <= 65535ms (got " +
                            std::to_string(intervalMs) + ")");
          }
        }
        if (trigger.contains("threshold")) {
          result.addError(
              label +
              " 'trigger.threshold' is not valid for 'periodic' triggers");
        }
      }
      ...
      if (triggerType == "onChange") {
        if (trigger.contains("threshold")) {
          if (!trigger["threshold"].is_number()) {
            result.addError(label + " 'trigger.threshold' must be a number");
          } else if (trigger["threshold"].get<float>() < 0.0f) {
            result.addError(label + " 'trigger.threshold' must be >= 0");
          }
        }
        if (trigger.contains("intervalMs")) {
          result.addError(
              label +
              " 'trigger.intervalMs' is not valid for 'onChange' triggers");
        }
      }
      ...
      if (triggerType == "onRequest") {
        if (trigger.contains("intervalMs")) {
          result.addError(
              label +
              " 'trigger.intervalMs' is not valid for 'onRequest' triggers");
        }
        if (trigger.contains("threshold")) {
          result.addError(
              label +
              " 'trigger.threshold' is not valid for 'onRequest' triggers");
        }
      }
    }
  }
  ...
  static void validateTelemetry(const json &data, ValidationResult &result) {
    ...
    for (size_t i = 0; i < data["telemetry"].size(); ++i) {
      ...
      if (source.contains("id") && source["id"].is_number_unsigned()) {
        const uint16_t id = source["id"].get<uint16_t>();
        if (seenIDs.count(id)) {
          result.addError("telemetry[" + std::to_string(i) +
                          "] has duplicate 'id' value (" + toHex(id) + ")");
        }
        seenIDs.insert(id);

        if (prevID > 0 && id != prevID + 1) {
          result.addWarning("Telemetry ID gap detected between " +
                            toHex(prevID) + " and " + toHex(id) +
  ...
  static void validateSettings(const json &data, ValidationResult &result) {
    ...
    for (size_t i = 0; i < data["settings"].size(); ++i) {
      ...
      if (!setting.contains("id") || !setting["id"].is_number_unsigned()) {
        ...
        if (id < MIN_ID) {
          result.addError(label + " has invalid 'id' value (" + toHex(id) +
                          "). Must be >= " + toHex(MIN_ID));
        }
        if (seenIDs.count(id)) {
        ...
        if (prevID > 0 && id != prevID + 1) {
          result.addWarning("Settings ID gap detected between " +
                            toHex(prevID) + " and " + toHex(id) +
                            " (this may be intentional)");
        }
        prevID = id;
      ...
      if (!setting.contains("type") || !setting["type"].is_string()) {
        ...
        if (!VALID_TYPES.count(type)) {
          result.addError(label + " has unsupported type '" + type +
                          "'. "
                          "Valid types are: UINT8, INT8, FLOAT, UINT16, INT16, "
                          "UINT32, INT32, STRING");
        }
      }

      if (!setting.contains("description") ||
          !setting["description"].is_string() ||
...
public:
  static ValidationResult validate(const json &data) {
    ...
    if (!data.contains("device") || !data["device"].is_string() ||
        data["device"].get<std::string>().empty()) {
    ...
    if (!std::isupper(deviceName[0])) {
      result.addError("Device name '" + deviceName +
                      "' should start with an uppercase letter (PascalCase)");
    }
    for (const char c : deviceName) {
      if (!std::isalnum(c)) {
        result.addError("'device' field must contain only alphanumeric "
                        "characters (no spaces or underscores)");
        break;
      }
    }

    if (!data.contains("typeShift") ||
        !data["typeShift"].is_number_unsigned()) {
      result.addError("'typeShift' field is missing. 'typeShift' field must be "
                      "an unsigned integer");
    } else {
    ...
    for (size_t i = 0; i < data["commands"].size(); ++i) {
      ...
      if (id < MIN_ID) {
        result.addError(cmdLabel + " has an invalid 'id' value (" + toHex(id) +
                        "). Must be >= " + toHex(MIN_ID));
      }
      ...
      if (cmd.contains("params") && cmd["params"].is_array()) {
        if (cmd["params"].size() > MAX_PARAMS) {
          result.addError(cmdLabel + " has more than " +
                          std::to_string(MAX_PARAMS) + " params");
        }
  ...
  validateCrossDevice(const std::vector<json> &allData) {
    ValidationResult result;
    ...
    for (const auto &data : allData) {
      ...
      const uint16_t shift =
          static_cast<uint16_t>(data["typeShift"].get<uint32_t>());
      auto it = seenShifts.find(shift);
      if (it != seenShifts.end()) {
        result.addError("Duplicate 'typeShift' value " + toHex(shift) +
                        ": device '" + deviceName + "' conflicts with '" +
                        it->second + "'");
      } else {
    ...
    for (const auto &data : allData) {
      ...
      if (data.contains("commands") && data["commands"].is_array()) {
        for (const auto &cmd : data["commands"]) {
          if (!cmd.contains("id") || !cmd.contains("name"))
            continue;
          const uint16_t rawId = cmd["id"].get<uint16_t>();
          const uint16_t builtID =
              buildCommandID(shift, CATEGORY_COMMAND, rawId);
          const std::string label =
              deviceName + "::" + cmd["name"].get<std::string>();

          auto it = seenCommandIDs.find(builtID);
          if (it != seenCommandIDs.end()) {
            result.addError("Cross-device command ID collision at shifted ID " +
                            toHex(builtID) + ": '" + label +
      ...
      if (data.contains("telemetry") && data["telemetry"].is_array()) {
        for (const auto &src : data["telemetry"]) {
          if (!src.contains("id") || !src.contains("name"))
            continue;
          const uint16_t builtID = buildCommandID(shift, CATEGORY_TELEMETRY,
                                                  src["id"].get<uint16_t>());
          const std::string label =
              deviceName + "::" + src["name"].get<std::string>();
          auto it = seenTelemetryIDs.find(builtID);
          if (it != seenTelemetryIDs.end()) {
            result.addError(
                "Cross-device telemetry ID collision at shifted ID " +
                toHex(builtID) + ": '" + label + "' conflicts with '" +
                it->second + "'");
          } else {
      ...
      if (data.contains("settings") && data["settings"].is_array()) {
        for (const auto &setting : data["settings"]) {
          if (!setting.contains("id") || !setting.contains("name"))
            continue;
          const uint16_t rawId = setting["id"].get<uint16_t>();
          const std::string label =
              deviceName + "::" + setting["name"].get<std::string>();

          const uint16_t getID =
              buildCommandID(shift, CATEGORY_SETTING_GET, rawId);
          const uint16_t setID =
              buildCommandID(shift, CATEGORY_SETTING_SET, rawId);

          auto itGet = seenCommandIDs.find(getID);
          if (itGet != seenCommandIDs.end()) {
            result.addError("Setting GET ID collision at " + toHex(getID) +
          ...
          if (itSet != seenCommandIDs.end()) {
            result.addError("Setting SET ID collision at " + toHex(setID) +
                            ": '" + label + "' conflicts with '" +
                            itSet->second + "'");
          } else {
