...
private:
  ...
  static uint16_t buildCommandID(uint16_t typeShift, uint8_t category,
                                 uint16_t localID) {
  ...
  static void createDirectory(const std::string &path) {
    ...
    // Walk the path, creating each segment that doesn't exist yet
    for (size_t i = 1; i <= p.size(); ++i) {
      if (i == p.size() || p[i] == '/') {
        const std::string segment = p.substr(0, i);
#if defined(_WIN32)
        if (_mkdir(segment.c_str()) != 0 && errno != EEXIST)
          throw std::runtime_error("Could not create directory: " + segment);
#else
  ...
  // Compute the minimum expected payload size for a command (required params
  // only)
  static size_t minPayloadSize(const json &cmd) {
    ...
    for (const auto &param : cmd["params"]) {
      ...
      if (type == "FLOAT")
        ...
        size += 2;
      // At minimum 2 bytes(1 for typeAndSize and null terminator for empty
      // string)
    }
    return size;
  }
    ...
    for (const auto &cmd : data["commands"]) {
      ...
      for (const auto &param : cmd["params"]) {
        ...
        if (type == "FLOAT")
          ...
          const size_t maxLen =
              param.contains("maxLen") ? param["maxLen"].get<size_t>() : 15;
  ...
  static std::string
  generateCommandTypesContent(const std::vector<json> &allData) {
    std::ostringstream out;
    out << "//\n// CommandTypes.h\n// AUTO-GENERATED BY SmartDrive "
           "CommandGenerator - DO NOT EDIT\n//\n\n";
    out << "#pragma once\n\n";
    out << "#include \"opx/shared/core/platform.h\"\n\n";
    ...
    for (const auto &data : allData) {
      ...
      for (const auto &cmd : data["commands"]) {
        ...
        const uint16_t id = buildCommandID(typeShift, CATEGORY_COMMAND,
                                           cmd["id"].get<uint16_t>());
        if (cmd.contains("description") &&
            !cmd["description"].get<std::string>().empty())
      ...
      if (data.contains("telemetry") && !data["telemetry"].empty()) {
        ...
        for (const auto &src : data["telemetry"]) {
          ...
          const uint16_t getID = buildCommandID(typeShift, CATEGORY_TELEMETRY,
                                                src["id"].get<uint16_t>());
          if (src.contains("description") &&
              !src["description"].get<std::string>().empty())
            out << "    // Request current value of: "
                << src["description"].get<std::string>() << "\n";
      ...
      if (data.contains("settings") && !data["settings"].empty()) {
        ...
        for (const auto &setting : data["settings"]) {
          ...
          const uint16_t getID =
              buildCommandID(typeShift, CATEGORY_SETTING_GET, localID);
          const uint16_t setID =
              buildCommandID(typeShift, CATEGORY_SETTING_SET, localID);
          if (setting.contains("description") &&
              !setting["description"].get<std::string>().empty())
            out << "    // Get current value of: "
                << setting["description"].get<std::string>() << "\n";
          out << "    constexpr uint16_t GET_SETTING_" << name << " = "
              << toHex(getID) << ";\n\n";
          if (setting.contains("description") &&
              !setting["description"].get<std::string>().empty())
            out << "    // Set value of: "
                << setting["description"].get<std::string>() << "\n";
          out << "    constexpr uint16_t SET_SETTING_" << name << " = "
              << toHex(setID) << ";\n\n";
        }
      }
  ...
  static std::string
  generateTelemetrySourceIDsContent(const std::vector<json> &allData) {
    std::ostringstream out;
    out << "//\n// TelemetrySourceIDs.h\n// AUTO-GENERATED BY SmartDrive "
           "CommandGenerator - DO NOT EDIT\n//\n\n";
    out << "#pragma once\n\n";
    out << "#include \"opx/shared/core/Config.h\" // IWYU pragma: keep\n\n";
    out << "#ifdef OPX_TARGET_EMBEDDED\n\n";
    out << "#include \"opx/shared/core/platform.h\"\n\n";
    ...
    out << "} // namespace TelemetrySource\n\n";
    out << "#endif // OPX_TARGET_EMBEDDED\n\n";
    return out.str();
  }

  static std::string
  generateSettingIDsContent(const std::vector<json> &allData) {
    std::ostringstream out;
    out << "//\n// SettingIDs.h\n// AUTO-GENERATED BY SmartDrive "
           "CommandGenerator - DO NOT EDIT\n//\n\n";
    out << "#pragma once\n\n";
  ...
  static std::string generateRegisterAllContent(const json &data) {
    ...
    out << "// AUTO-GENERATED BY SmartDrive CommandGenerator - DO NOT "
           "EDIT\n//\n\n";
    out << "#pragma once" << "\n\n";
    out << "#include \"opx/shared/core/Config.h\" // IWYU pragma: keep\n\n";
    out << "#ifdef OPX_TARGET_EMBEDDED\n\n";
    out << "#include \"opx/shared/core/platform.h\"\n";
    out << "#include \"opx/embedded/core/OpxDevice.h\"\n";
    out << "#include \"opx/shared/core/TriggerConfig.h\"\n";
    out << "#include \"../../SettingIDs.h\"\n";
    out << "#include \"../../TelemetrySourceIDs.h\"\n\n";

    // ── Settings registration ─────────────────────────────────────────────
    out << "inline void register" << deviceName
        << "Settings(OpxDevice& device) {\n";
    if (!data.contains("settings") || data["settings"].empty()) {
      ...
      for (const auto &setting : data["settings"]) {
        const std::string name = setting["name"].get<std::string>();
        const std::string type = setting["type"].get<std::string>();
        out << "    device.registerSetting(SettingID::" << deviceName
            << "Setting::" << name << ", " << valueTypeEnum(type) << ");\n";
      }
    }
    out << "}\n\n";
  ...
  static std::string generateRegisterDeviceContent(const json &data) {
    ...
    out << "#include \"opx/shared/core/platform.h\"\n";
    out << "#include \"opx/embedded/core/OpxDevice.h\"\n";
    out << "#include \"" << "../../../shared/devices/" << deviceName << "/"
        << deviceName << "Controller.h\"\n";
    ...
    out << "    device.setTypeShift(" << deviceName
        << "Controller::TYPE_ID);\n";
  ...
  static std::string
  generateCommandPackerContent(const std::vector<json> &allData) {
    std::ostringstream out;
    ...
    out << "#include \"opx/shared/core/platform.h\"\n";
    out << "#include \"opx/shared/types/ProtocolTypes.h\"\n\n";
    ...
    out << "            case 0xFF00: {\n";
    out << "                // Protocol-level GET_ALL_SETTINGS — no "
           "parameters\n";
    out << "                return offset;\n";
    out << "            }\n\n";

    out << "            case 0xFD00: {\n";
    out << "                // Protocol-level DISCOVER — no parameters\n";
    out << "                return offset;\n";
    out << "            }\n\n";

    out << "            case 0xFD01: {\n";
    out << "                // Protocol-level ANNOUNCE — one uint8_t "
           "(typeShift)\n";
    out << "                memcpy(&buffer[offset], cmd.params[0].getData(), "
           "sizeof(uint8_t));\n";
    ...
    out << "            case 0xFC01: {\n";
    out << "                // Protocol-level HEARTBEAT_ACK — one uint8_t "
           "(typeShift)\n";
    out << "                memcpy(&buffer[offset], cmd.params[0].getData(), "
           "sizeof(uint8_t));\n";
    out << "                offset += sizeof(uint8_t);\n";
        ...
        if (!cmd.contains("params") || cmd["params"].empty()) {
          ...
          for (size_t i = 0; i < cmd["params"].size(); ++i) {
            ...
            if (type == "STRING") {
              ...
              out << "buffer[offset++] = cmd.params[" << i
                  << "].getTypeAndSize();\n";
              if (isOptional)
                out << "                    ";

              // Then write string data
              out << "const size_t strDataSize" << i << " = cmd.params[" << i
                  << "].getDataSize();\n";
              if (isOptional)
                out << "                    ";
              out << "memcpy(&buffer[offset], cmd.params[" << i
                  << "].getData(), strDataSize" << i << ");\n";
              if (isOptional)
                out << "                    ";
              out << "offset += strDataSize" << i << ";\n";
            } else {
              out << "memcpy(&buffer[offset], cmd.params[" << i
                  << "].getData(), " << sizeofForType(type) << ");\n";
              if (isOptional)
                out << "                    ";
    ...
    // Auto-generated GET commands — no parameters, just return offset after
    // writing commandType
    for (const auto &data : allData) {
      if (!data.contains("telemetry") || data["telemetry"].empty())
        continue;
      ...
      for (const auto &src : data["telemetry"]) {
        ...
        out << "            case " << deviceName << "CommandType::GET_" << name
            << ": {\n";
        out << "                // Auto-generated telemetry request — no "
               "parameters\n";
        out << "                return offset;\n";
        out << "            }\n\n";
      }
    ...
    for (const auto &data : allData) {
      ...
      for (const auto &setting : data["settings"]) {
        ...
        // GET — no parameters
        out << "            case " << deviceName << "CommandType::GET_SETTING_"
            << name << ": {\n";
        out << "                // Auto-generated setting GET — no "
               "parameters\n";
        out << "                return offset;\n";
        out << "            }\n\n";

        // SET — one parameter of the setting's type
        out << "            case " << deviceName << "CommandType::SET_SETTING_"
            << name << ": {\n";
        out << "                // Auto-generated setting SET — one "
               "parameter\n";
        if (type == "STRING") {
          out << "                buffer[offset++] = "
                 "cmd.params[0].getTypeAndSize();\n";
          out << "                const size_t strDataSize = "
                 "cmd.params[0].getDataSize();\n";
          out << "                memcpy(&buffer[offset], "
                 "cmd.params[0].getData(), strDataSize);\n";
          out << "                offset += strDataSize;\n";
        } else {
          out << "                memcpy(&buffer[offset], "
                 "cmd.params[0].getData(), "
              << sizeofForType(type) << ");\n";
          out << "                offset += " << sizeofForType(type) << ";\n";
        }
        out << "                return offset;\n";
    ...
    out << "    static bool unpack(const uint8_t* buffer, size_t bufferSize, "
           "Command& cmdOut) {\n";
    ...
    out << "                // Protocol-level ANNOUNCE — one uint8_t "
           "(typeShift)\n";
    out << "                if (bufferSize - offset < sizeof(uint8_t)) return "
           "false;\n";
    out << "                cmdOut.params[0] = uint8_t(0);\n";
    out << "                memcpy(cmdOut.params[0].getDataMutable(), "
    ...
    for (const auto &data : allData) {
      ...
      for (const auto &cmd : data["commands"]) {
        ...
        if (!cmd.contains("params") || cmd["params"].empty()) {
          ...
          out << "                if (bufferSize < " << minSize
              << ") return false;\n";
          out << "                size_t remainingBytes = bufferSize - "
                 "offset;\n\n";
          ...
          for (size_t i = 0; i < cmd["params"].size(); ++i) {
            ...
            const std::string defaultValue =
                param.contains("default") ? param["default"].get<std::string>()
                                          : "";
            ...
            if (isOptional) {
              ...
              if (type == "STRING") {
                ...
                out << "                    const uint8_t typeAndSize" << i
                    << " = buffer[offset++];\n";
                out << "                    cmdOut.params[" << i
                    << "].setTypeAndSizeRaw(typeAndSize" << i << ");\n";
                out << "                    remainingBytes--;\n";
                out << "                    \n";
                out << "                    // Now read string data\n";
                out << "                    const size_t strSize" << i
                    << " = cmdOut.params[" << i << "].getDataSize();\n";
                out << "                    if (remainingBytes < strSize" << i
                    << ") return false;\n";
              ...
              if (type == "STRING") {
                out << "                {\n";
                out << "                    // Read typeAndSize byte first\n";
                out << "                    if (remainingBytes < 1) return "
                       "false;\n";
                out << "                    const uint8_t typeAndSize" << i
                    << " = buffer[offset++];\n";
                out << "                    cmdOut.params[" << i
                ...
                out << "                if (remainingBytes < "
                    << sizeofForType(type) << ") return false;\n";
                out << "                memcpy(cmdOut.params[" << i
                    << "].getDataMutable(), &buffer[offset], "
                    << sizeofForType(type) << ");\n";
                out << "                offset += " << sizeofForType(type)
    ...
    for (const auto &data : allData) {
      ...
      for (const auto &setting : data["settings"]) {
        ...
        out << "            case " << deviceName << "CommandType::SET_SETTING_"
            << name << ": {\n";
        out << "                if (bufferSize < 2 + " << sizeofForType(type)
            << ") return false;\n";
        out << "                cmdOut.params[0] = " << defVal << ";\n";
        if (type == "STRING") {
          out << "                if (bufferSize < 3) return false;\n";
          out << "                const uint8_t typeAndSize = "
                 "buffer[offset++];\n";
          out << "                "
                 "cmdOut.params[0].setTypeAndSizeRaw(typeAndSize);\n";
          out << "                const size_t strSize = "
                 "cmdOut.params[0].getDataSize();\n";
          out << "                if (bufferSize - offset < strSize) return "
                 "false;\n";
          out << "                memcpy(cmdOut.params[0].getDataMutable(), "
                 "&buffer[offset], strSize);\n";
          out << "                offset += strSize;\n";
        } else {
          out << "                if (bufferSize - offset < "
              << sizeofForType(type) << ") return false;\n";
          out << "                memcpy(cmdOut.params[0].getDataMutable(), "
                 "&buffer[offset], "
              << sizeofForType(type) << ");\n";
    ...
    out << "    // packedSize() and stringParamOffset() are kept in sync with "
           "pack() by the generator.\n";
    out << "    // When adding a new command to the JSON, regenerate to update "
           "all three.\n\n";
    out << "    // Returns total bytes that pack() writes for the given "
           "commandType,\n";
    out << "    // INCLUDING the 2-byte commandType prefix.\n";
    out << "    // Returns 0xFF (STRING_SENTINEL) if the command has a string "
           "parameter.\n";
    ...
    // Auto-generated GET commands — always 2 bytes (commandType only, no
    // params)
    for (const auto &data : allData) {
      if (!data.contains("telemetry") || data["telemetry"].empty())
        continue;
    ...
    for (const auto &data : allData) {
      ...
      for (const auto &setting : data["settings"]) {
        ...
        if (type == "STRING") {
          out << "            case " << deviceName
              << "CommandType::SET_SETTING_" << name << ": return 0xFF;\n";
        } else {
          out << "            case " << deviceName
              << "CommandType::SET_SETTING_" << name << ": return "
              << (COMMAND_TYPE_SIZE + sizeofForTypeValue(type)) << ";\n";
        }
      }
    }
    ...
    out << "    // Returns the byte offset from the start of the payload "
           "(after the header byte)\n";
    out << "    // where the string parameter's typeAndSize byte can be "
           "found.\n";
    ...
    for (const auto &data : allData) {
      ...
      for (const auto &cmd : data["commands"]) {
        ...
        // Offset = seqNum(1) + commandType(2) + all fixed params before the
        // string
        bool hasString = false;
        size_t offset =
            3; // seqNum(1) + commandType(2) — always present before params
  ...
  static std::string
  generateCommandRegistryContent(const std::vector<json> &allData) {
    std::ostringstream out;
    ...
    out << "// AUTO-GENERATED BY SmartDrive CommandGenerator - DO NOT EDIT\n";
    out << "//\n\n";
    out << "#include \"opx/pc/registry/CommandRegistry.h\"\n";
    out << "#include \"autogen/shared/CommandTypes.h\"\n";
    out << "#include \"opx/shared/core/Config.h\" // IWYU pragma: keep\n\n";
    out << "#ifndef OPX_TARGET_EMBEDDED\n\n";
        ...
        if (cmd.contains("params")) {
          for (const auto &param : cmd["params"]) {
            ...
            const size_t maxLength =
                (type == "STRING" && param.contains("max_length"))
            ...
            out << "            {\"" << paramName << "\", "
                << valueTypeEnum(type) << ", " << (required ? "true" : "false")
                << ", "
                << "\"" << paramDesc << "\", " << maxLength << ", "
                << "\"" << defVal << "\"},\n";
          }
  ...
  static std::string generateControllerContent(const json &data) {
    ...
    out << "#pragma once" << "\n\n";
    out << "#include \"opx/shared/core/platform.h\"\n";
    out << "#include \"opx/shared/core/CommunicationManager.h\"\n";
    out << "#include \"opx/shared/types/ProtocolTypes.h\"\n";
    out << "#include \"../../CommandTypes.h\"\n\n";
    out << "class " << className << " {\n";
    out << "private:\n";
    ...
    out << "    explicit " << className
        << "(CommunicationManager& comms) : comms(comms) {}\n\n";
    ...
    for (const auto &cmd : data["commands"]) {
      ...
      out << "        Command cmd;\n";
      out << "        cmd.commandType = " << deviceName
          << "CommandType::" << cmdName << ";\n";
      ...
      if (cmd.contains("params") && !cmd["params"].empty()) {
        for (size_t i = 0; i < cmd["params"].size(); ++i) {
          ...
          if (isRequired) {
            ...
            // But since ValueSource assignment always sets a type, we assign
            // directly
            out << "        cmd.params[" << i << "] = " << pName << ";\n";
          }
        }
    ...
    if (data.contains("telemetry") && !data["telemetry"].empty()) {
      ...
      for (const auto &src : data["telemetry"]) {
        ...
        out << "        Command cmd;\n";
        out << "        cmd.commandType = " << deviceName << "CommandType::GET_"
            << srcName << ";\n";
    ...
    if (data.contains("settings") && !data["settings"].empty()) {
      ...
      for (const auto &setting : data["settings"]) {
        ...
        out << "        Command cmd;\n";
        out << "        cmd.commandType = " << deviceName
            << "CommandType::GET_SETTING_" << settingName << ";\n";
        out << "        return comms.dispatch(cmd, transportID, false);\n";
        out << "    }\n\n";
        ...
        out << "    bool set" << pascal << "(" << cppType
            << " value, uint8_t transportID = "
               "ProtocolConstants::TRANSPORT_ID_DEFAULT) {\n";
        out << "        Command cmd;\n";
        out << "        cmd.commandType = " << deviceName
            << "CommandType::SET_SETTING_" << settingName << ";\n";
  ...
  static std::string
  generateProtocolConfigContent(const std::vector<json> &allData) {
    ...
    out << "#pragma once\n\n";
    out << "#include \"opx/shared/core/platform.h\"\n";
    out << "// Maximum packed parameter bytes across all commands in this "
           "device.\n";
  ...
  static std::string
  generateDeviceManifestContent(const std::vector<json> &allData) {
    std::ostringstream out;

    out << "//\n";
    out << "// DeviceManifest.h\n";
    out << "// AUTO-GENERATED BY SmartDrive CommandGenerator - DO NOT EDIT\n";
    out << "//\n";
    out << "// Maps each device's typeShift to its display name and the list "
           "of\n";
    out << "// command IDs it supports. Used by the PC console to build the\n";
    out << "// device/command menus at runtime without needing compile-time "
           "type knowledge.\n";
    out << "//\n\n";
    out << "#pragma once\n\n";
    out << "#include \"opx/shared/core/Config.h\" // IWYU pragma: keep\n\n";
    out << "#ifdef OPX_TARGET_PC\n\n";
    out << "#include <cstdint>\n";
    out << "#include <vector>\n";
    out << "#include <string>\n";
    out << "#include \"../shared/CommandTypes.h\"\n\n";
    out << "struct DeviceManifestEntry {\n";
    out << "    uint8_t               typeShift;\n";
    out << "    const char*           displayName;\n";
    out << "    std::vector<uint16_t> commandIDs;\n";
    out << "};\n\n";
    out << "// Returns the full device manifest. Generated once as a static "
           "local.\n";
    out << "// Add one entry per device.json in your project.\n";
    out << "inline const std::vector<DeviceManifestEntry>& getDeviceManifest() "
           "{\n";
    out << "    static const std::vector<DeviceManifestEntry> manifest = {\n";

    for (size_t d = 0; d < allData.size(); ++d) {
      const auto &data = allData[d];
      if (!data.contains("device") || !data.contains("typeShift"))
        continue;
      ...
      if (data.contains("settings") && data["settings"].is_array() &&
          !data["settings"].empty()) {
        out << "                // auto-generated setting GET/SET commands\n";
        for (const auto &setting : data["settings"]) {
          if (!setting.contains("name"))
            continue;
          const std::string name = setting["name"].get<std::string>();
          out << "                " << deviceName << "CommandType::GET_SETTING_"
              << name << ",\n";
          out << "                " << deviceName << "CommandType::SET_SETTING_"
              << name << ",\n";
        }
      }
    ...
    out << "    return manifest;\n";
    out << "}\n\n";
    out << "#endif // OPX_TARGET_PC\n\n";

    return out.str();
  }

  static std::string
  generateOpxDevicesContent(const std::vector<json> &allData) {
    std::ostringstream out;
    out << "//\n// OpxDevices.h\n";
    out << "// AUTO-GENERATED BY OmniPlexus CommandGenerator - DO NOT EDIT\n";
    out << "// Contains device-specific includes derived from your device JSON "
           "files.\n";
    out << "//\n\n";
    out << "#pragma once\n";
    ...
    for (const auto &data : allData) {
      const std::string deviceName = data["device"].get<std::string>();
      out << "// " << deviceName << "\n";
      out << "#include \"devices/" << deviceName << "/" << deviceName
          << "Controller.h\"\n";
      out << "#include \"../embedded/devices/" << deviceName << "/"
          << deviceName << "RegisterAll.h\"\n";
      out << "#include \"../embedded/devices/" << deviceName << "/"
          << deviceName << "Register.h\"\n";
      out << "\n";
...
public:
  ...
  static void generate(const std::vector<json> &allData,
    ...
    auto ensureSlash = [](std::string p) -> std::string {
      if (!p.empty() && p.back() != '/')
        p += '/';
      return p;
    };

    const std::string sharedDir = ensureSlash(headerOutputDir) + "shared/";
    const std::string embeddedDir = ensureSlash(headerOutputDir) + "embedded/";
    const std::string pcDir = ensureSlash(headerOutputDir) + "pc/";
    const std::string sourceDir = ensureSlash(sourceOutputDir);

    // Create shared/ and devices/ directories
    createDirectory(sharedDir);
    createDirectory(embeddedDir);
    createDirectory(pcDir);
    createDirectory(sourceDir);

    // ── Shared files ──────────────────────────────────────────────────────

    writeFile(sharedDir + "CommandTypes.h",
              generateCommandTypesContent(allData));
    std::cout << "  ✓ Generated: " << sharedDir << "CommandTypes.h\n";

    writeFile(sharedDir + "CommandPacker.h",
              generateCommandPackerContent(allData));
    std::cout << "  ✓ Generated: " << sharedDir << "CommandPacker.h\n";

    writeFile(sourceDir + "CommandRegistry.cpp",
              generateCommandRegistryContent(allData));
    std::cout << "  ✓ Generated: " << sourceDir << "CommandRegistry.cpp\n";

    writeFile(sharedDir + "GeneratedConfig.h",
              generateProtocolConfigContent(allData));
    std::cout << "  ✓ Generated: " << sharedDir << "GeneratedConfig.h\n";

    writeFile(sharedDir + "OpxDevices.h", generateOpxDevicesContent(allData));
    std::cout << "  ✓ Generated: " << sharedDir << "OpxDevices.h\n";

    writeFile(pcDir + "DeviceManifest.h",
              generateDeviceManifestContent(allData));
    std::cout << "  ✓ Generated: " << pcDir << "DeviceManifest.h\n";
    ...
    if (anyTelemetry) {
      writeFile(embeddedDir + "TelemetrySourceIDs.h",
                generateTelemetrySourceIDsContent(allData));
      std::cout << "  ✓ Generated: " << embeddedDir << "TelemetrySourceIDs.h\n";
    }
    ...
    if (anySettings) {
      writeFile(embeddedDir + "SettingIDs.h",
                generateSettingIDsContent(allData));
      std::cout << "  ✓ Generated: " << embeddedDir << "SettingIDs.h\n";
    }

    // ── Per-device files ──────────────────────────────────────────────────
    for (const auto &data : allData) {
      const std::string deviceName = data["device"].get<std::string>();
      const std::string sharedDeviceDir =
          sharedDir + "devices/" + deviceName + "/";
      const std::string embeddedDeviceDir =
          embeddedDir + "devices/" + deviceName + "/";
      createDirectory(sharedDeviceDir);
      createDirectory(embeddedDeviceDir);

      writeFile(sharedDeviceDir + deviceName + "Controller.h",
                generateControllerContent(data));
      std::cout << "  ✓ Generated: " << sharedDeviceDir << deviceName
                << "Controller.h\n";

      writeFile(embeddedDeviceDir + deviceName + "RegisterAll.h",
                generateRegisterAllContent(data));
      std::cout << "  ✓ Generated: " << embeddedDeviceDir << deviceName
                << "RegisterAll.h\n";

      writeFile(embeddedDeviceDir + deviceName + "Register.h",
                generateRegisterDeviceContent(data));
      std::cout << "  ✓ Generated: " << embeddedDeviceDir << deviceName
                << "Register.h\n";
    }
  }
