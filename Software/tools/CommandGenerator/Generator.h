...
private:
  ...
  static std::string generateRegisterAllContent(const json &data) {
    ...
    out << "#include \"../../shared/SettingIDs.h\"\n";
    out << "#include \"../../shared/TelemetrySourceIDs.h\"\n\n";
  ...
  static std::string generateRegisterDeviceContent(const json &data) {
    ...
    out << "#include \"opx/core/platform.h\"\n";
    out << "#include \"opx/core/OpxDevice.h\"\n";
    out << "#include \"" << deviceName << "Controller.h\"\n";
  ...
  generateCommandRegistryContent(const std::vector<json> &allData) {
    ...
    out << "#include \"opx/generated/shared/CommandTypes.h\"\n\n";
...
public:
  ...
  static void generate(const std::vector<json> &allData,
    ...
    const std::string devicesDir = ensureSlash(headerOutputDir) + "devices/";
    ...
    writeFile(sharedDir + "DeviceManifest.h",
              generateDeviceManifestContent(allData));
    std::cout << "  ✓ Generated: " << sharedDir << "DeviceManifest.h\n";
    ...
    if (anyTelemetry) {
      writeFile(sharedDir + "TelemetrySourceIDs.h",
                generateTelemetrySourceIDsContent(allData));
      std::cout << "  ✓ Generated: " << sharedDir << "TelemetrySourceIDs.h\n";
    }
    ...
    if (anySettings) {
      writeFile(sharedDir + "SettingIDs.h", generateSettingIDsContent(allData));
      std::cout << "  ✓ Generated: " << sharedDir << "SettingIDs.h\n";
    }
    ...
    for (const auto &data : allData) {
      const std::string deviceName = data["device"].get<std::string>();
      const std::string deviceDir = devicesDir + deviceName + "/";
      createDirectory(deviceDir);

      writeFile(deviceDir + deviceName + "Controller.h",
                generateControllerContent(data));
      std::cout << "  ✓ Generated: " << deviceDir << deviceName
                << "Controller.h\n";

      writeFile(deviceDir + deviceName + "RegisterAll.h",
                generateRegisterAllContent(data));
      std::cout << "  ✓ Generated: " << deviceDir << deviceName
                << "RegisterAll.h\n";

      writeFile(deviceDir + deviceName + "Register.h",
