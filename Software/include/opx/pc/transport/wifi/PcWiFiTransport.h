...
public:
  explicit PcWiFiTransport(uint16_t port)
      : role(WifiRole::SERVER), socket(ioContext), maxReconnectAttempts(0),
        reconnectDelayMs(0) {
  ...
  PcWiFiTransport(const char *host, uint16_t port,
      ...
      : role(WifiRole::CLIENT), socket(ioContext), targetHost(host),
        targetPort(port), maxReconnectAttempts(maxReconnectAttempts),
    ...
    if (tryConnect()) {
      ...
      LOG(LogLevel::OP_WARNING,
          "WiFi CLIENT: initial connection failed, will retry in accumulate()");
    }
  }
  ...
  bool isConnected() const override { return clientConnected.load(); }

  bool send(const SerializedData &data) override {
    if (!clientConnected) {
      LOG(LogLevel::OP_WARNING,
          "WiFi transport: send() called but not connected");
      return false;
    }
    try {
      std::lock_guard<std::mutex> lock(socketMutex);
  ...
  void accumulate() override {
    ...
    if (role == WifiRole::CLIENT && !clientConnected) {
      ...
      const auto elapsed =
          std::chrono::duration_cast<std::chrono::milliseconds>(
              now - lastReconnectAttemptTime)
              .count();

      if (elapsed < static_cast<long>(reconnectDelayMs))
        return;
      ...
      if (reconnectAttempts > maxReconnectAttempts) {
        LOG(LogLevel::OP_ERROR,
            "WiFi CLIENT: max reconnect attempts reached, giving up");
        giveUp = true;
        return;
      }
    ...
    bool disconnected = false;
    {
      ...
      if (!socket.is_open()) {
        ...
        uint8_t peek;
        socket.receive(boost::asio::buffer(&peek, 1),
                       boost::asio::ip::tcp::socket::message_peek, ec);
...
protected:
  uint16_t bytesAvailable() override {
    ...
    const size_t bytesRead =
        socket.read_some(boost::asio::buffer(stagingBuffer, toRead), ec);
