...
class PcHttpTransport : public ITransport {
  ...
  std::string targetPath = "/opx"; // POST endpoint

public:
  // server runs on a background thread so main loop can remain free
  explicit PcHttpTransport(uint16_t port)
      : role(HttpRole::SERVER), listenPort(port) {
    svr.Post(targetPath.c_str(), [this](const httplib::Request &req,
      ...
      if (incoming > ProtocolConstants::MAX_FRAME_SIZE) {
        LOG(LogLevel::OP_WARNING,
            "HTTP SERVER: incoming frame too large, dropping");
        res.status = 413;
        return;
      }

      {
        ...
        if (slot.ready) { // prevous frame has not been consumed yet
          LOG(LogLevel::OP_WARNING,
              "HTTP SERVER: previous frame not consumed, dropping new frame");
          res.status = 503;
          return;
        }
  ...
  bool send(const SerializedData &data) override {
    if (role == HttpRole::CLIENT) {
      ...
      auto res = cli->Post(targetPath.c_str(),
                           reinterpret_cast<const char *>(data.data), data.size,
                           "application/octet-stream");
      if (!res) {
        LOG(LogLevel::OP_ERROR, httplib::to_string(res.error()).c_str());
        LOG(LogLevel::OP_ERROR,
            "HTTP CLIENT: request failed (no response/timeout)");
        return false;
      }
      if (res->status != 200) {
        LOG(LogLevel::OP_ERROR, ("HTTP CLIENT: server responded with status " +
      ...
      if (!res->body.empty()) {
        ...
        if (bodySize > ProtocolConstants::MAX_FRAME_SIZE) {
          LOG(LogLevel::OP_WARNING,
              "HTTP CLIENT: response frame too large, ignoring body");
          return true; // We consider this a success since the request itself
                       // succeeded
        }
        std::lock_guard<std::mutex> lock(slot.mtx);
        if (!slot.ready) {
    ...
    LOG(LogLevel::OP_WARNING,
        "HTTP SERVER: send() called but transport is in SERVER mode, ignoring");
    return false;
  }
