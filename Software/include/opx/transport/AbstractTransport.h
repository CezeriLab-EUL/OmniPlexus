...
private:
  ...
  ProtocolConstants::FrameType currentFrameType =
...
private:
  uint8_t computeRemainingPayload() {
    ...
    if (currentFrameType == ProtocolConstants::FrameType::COMMAND) {
      // Buffer layout: [0]=header, [1]=seqNum, [2..3]=commandType
      // (little-endian)
      ...
      // pack() output includes commandType(2) at front; preamble already read
      // it. remainder = everything pack() writes after the commandType prefix.
  ...
  void processByte(const uint8_t byte) {
    ...
    case AccumulatorState::READING_PREAMBLE: {
      ...
      if (bytesCollected == 1 + preambleTarget) {
        ...
        if (remaining == ProtocolConstants::STRING_SENTINEL) {
          ...
          // Bytes already collected after header = preambleTarget (=
          // COMMAND_PREAMBLE_SIZE = 3).
    ...
    case AccumulatorState::VALIDATING_CRC: {
      ...
      accumulateBuffer[bytesCollected++] = byte;
      RawData toCheck{accumulateBuffer,
                      static_cast<size_t>(bytesCollected - 1)};
      ...
      if (receivedCRC != calculatedCRC) {
        ...
        if (!frameConsumed) {
          LOG(LogLevel::OP_WARNING,
              "Previous frame not released, dropping new frame");
          reset();
        } else {
