...
// ── Timing constants ──────────────────────────────────────────────────────────
// Debounce: ignore edges arriving too close together (noise rejection)
// Must be less than CLK_HALF_US * 2 = 400µs (minimum valid inter-edge time)
...
// ── GPIO macros ───────────────────────────────────────────────────────────────
// These are set up in beginCDnC() based on dataPin. Direct port manipulation
// for ISR speed — must not use digitalWrite() in the ISR.
// PD4 = Arduino pin 4 = DATA, PD3 = Arduino pin 3 = CLK (INT1)
...
public:
    ...
    bool begin(uint8_t dataPin, uint8_t clkPin) {
        _dataPin    = dataPin;
        _clkPin     = clkPin;
        _dataPinBit = digitalPinToBitMask(dataPin) == 0 ? dataPin : __builtin_ctz(digitalPinToBitMask(dataPin));
...
private:
    ...
    // ── TX queue helper ───────────────────────────────────────────────────────
    uint8_t txPop() {
        if (_txQueueRead == _txQueueWrite) return 0xFF;  // NOP = release line
    ...
    static uint8_t minFrameSize(uint8_t header) {
        const ProtocolConstants::FrameType ft = ProtocolConstants::decodeType(header);
        switch (ft) {
