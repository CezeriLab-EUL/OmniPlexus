...
//   • NOP pad uses |= set_mask (release bit HIGH), not &= clear_mask (drive
//   LOW) • valid = (~start_bits) & (~end_bits) — LOW = valid, HIGH = absent •
//   cdnc_gpio_rx_mode sets PUPDR=0 — no internal pull-downs (they fight 5V
//   pull-ups) • GPIOE clock enabled BEFORE register writes (silently ignored
//   otherwise) • JTAG disabled on F103 to free PB3/PB4/PA15 for GPIO
...
static void cdnc_gpio_rx_mode(void) {
  ...
  GPIOE->PUPDR =
      0x00000000; // NO internal pull-downs — external 5V pull-ups only
...
static void cdnc_update_detection(uint16_t valid_mask) {
  ...
  for (uint8_t s = 0; s < CDNC_MAX_SLAVES; s++) {
    ...
    if (saw_silent[s] && bit == 1) {
      ...
      next =
          (recent >= DETECT_THRESHOLD) ? CDNC_SLAVE_ONLINE : CDNC_SLAVE_OFFLINE;
      // Mark saw_silent when online slave misses an exchange
      if (prev == CDNC_SLAVE_ONLINE && bit == 0) {
        saw_silent[s] = true;
      }
      if (next == CDNC_SLAVE_OFFLINE)
        saw_silent[s] = false;
    }
...
// ── TX pad
// ──────────────────────────────────────────────────────────────────── When a
// slave's TX queue falls behind the read pointer, fill with 1s (release).
...
void cdnc_init(void) {
  ...
  AFIO->MAPR =
      (AFIO->MAPR & ~AFIO_MAPR_SWJ_CFG) | AFIO_MAPR_SWJ_CFG_JTAGDISABLE;
  ...
  for (int i = 0; i < CDNC_TX_BUF_SIZE; i++)
    txBuf[i] = 0xFFFF;
...
void setCdncSlotMask(uint16_t mask) { _cdncSlotMask = mask; }

bool cdnc_send_byte(uint8_t slave, uint8_t b) {
  if (slave >= CDNC_MAX_SLAVES)
    return false;

  uint32_t w = writePtr[slave];
  uint32_t r = readPtr;
  if ((w - r) + 8 > CDNC_TX_BUF_SIZE)
    return false;
...
uint16_t cdnc_alive_mask(void) {
  uint16_t mask = 0;
  for (uint8_t s = 0; s < CDNC_MAX_SLAVES; s++) {
    if (slave_state[s] == CDNC_SLAVE_ONLINE)
      mask |= (1U << s);
  }
