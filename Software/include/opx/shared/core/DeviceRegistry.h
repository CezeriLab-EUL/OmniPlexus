...
public:
  using DeviceConnectedCallback = void (*)(uint8_t typeShift,
                                           uint8_t transportID, void *context);
  using DeviceDisconnectedCallback = void (*)(uint8_t typeShift,
                                              uint8_t transportID,
                                              void *context);
...
public:
  ...
  void handleAnnounce(uint8_t typeShift, uint8_t transportID) {
    ...
    if (count >= MAX_DISCOVERED_DEVICES) {
      LOG(LogLevel::OP_ERROR, "DeviceRegistry: Max device limit reached. "
  ...
  void handleDisconnect(uint8_t typeShift) {
    ...
    if (idx < 0) {
      LOG(LogLevel::OP_WARNING,
          "DeviceRegistry: Received disconnect for unknown device");
      return;
    }
  ...
  void checkTimeouts() {
    ...
    for (uint8_t i = 0; i < MAX_DISCOVERED_DEVICES; i++) {
      if (!entries[i].active)
        continue;
      if (entries[i].lastSeenMs == 0)
        continue; // never seen yet — skip
      ...
      if (elapsed >= deviceTimeoutMs) {
        ...
        count--;
        if (disconnectedCallback) {
          disconnectedCallback(typeShift, transportID,
                               disconnectedCallbackContext);
        }
      }
    }
  }
  ...
  void onDeviceDisconnected(DeviceDisconnectedCallback cb,
  ...
  void removeByTransport(uint8_t transportID) {
    for (uint8_t i = 0; i < MAX_DISCOVERED_DEVICES; i++) {
      if (entries[i].active && entries[i].transportID == transportID) {
        if (disconnectedCallback) {
          disconnectedCallback(entries[i].typeShift, transportID,
                               disconnectedCallbackContext);
        }
        entries[i].active = false;
        count--;
  ...
  void clear() {
    for (uint8_t i = 0; i < MAX_DISCOVERED_DEVICES; i++) {
      if (entries[i].active && disconnectedCallback) {
        disconnectedCallback(entries[i].typeShift, entries[i].transportID,
                             disconnectedCallbackContext);
      }
      entries[i].active = false;
    }
