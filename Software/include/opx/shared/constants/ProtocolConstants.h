...
constexpr uint8_t TYPE_AND_SIZE_BYTE =
    1; // 1 byte for ValueSource type and size

constexpr uint16_t MAX_PAYLOAD_SIZE = 64;
constexpr uint8_t CRC_OFFSET = 1;
constexpr uint8_t NOP_BYTE =
    0x00; // Sent when clocking in data with nothing to transmit
...
constexpr uint8_t RESPONSE_PREAMBLE_SIZE =
    0; // Size can be known from header alone
...
// returned by the command packer for commands that have string params (don't
// forget to update the generator if this value is modified)
...
constexpr bool isProtocolLevelCommand(uint16_t cmdType) {
  return cmdType == GET_ALL_SETTINGS_COMMAND || cmdType == DISCOVER_COMMAND ||
         cmdType == ANNOUNCE_COMMAND || cmdType == HEARTBEAT_COMMAND ||
